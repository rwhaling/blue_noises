...
const BLUR_PASSES = 3;   // Example blur passes
...
let WAVE_AMPLITUDE = 0.2;
let WAVE_XSCALE = 3.5;      // NEW: x scale for the wave
let WAVE_TIMESCALE = 0.3;   // NEW: time scale for the wave

// Palette Colors
let GRADIENT_COLOR_A = '#F8B55F'; // Base Color A
let GRADIENT_COLOR_B = '#C95792'; // Base Color B
// --- Step 1: Add New Color Constants ---
const PALETTE_COLOR_C = '#AC1138'; // Color for opaque black elements
const PALETTE_COLOR_D = '#000000'; // Color for opaque white elements (unused for now)
...
export function draw({ /*canvas2d,*/ canvasWebGL }: CanvasContexts) {
    ...
    elementsCtx.arc(elementsCanvas.width / 2, elementsCanvas.height / 2, 150, 0, Math.PI * 2);
    elementsCtx.fill();
